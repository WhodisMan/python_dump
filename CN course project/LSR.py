...
def check_choices(command):
    if not command.isdigit():
        print("Please enter a number as a command from the given choices..")
        return -1
    else:
        command = int(command)

        if command > 4 or command < 1:
            print("Please enter a valid command from the given choices..")
            return -1
        else:
            return command
...
def process_file(fname):
    ...
    with open(fname) as f:
        router_matrix = [list(map(int, x.split(" "))) for x in
                         f]  # Data from the input file is stored in a two-dimensional list(array).
    matrix_set = 1
    ...
    set_distances(
        router_matrix)  # Distances are stored in a dictionary - key,value pair - with source router as key and distances in the form of a dictionary as value.
...
# Function to generate the shortest path using the parent table generated by the function dijkstra.
...
while command != 4:
    ...
    # Accept the source router and display the connection table.
        ...
        if matrix_set == 1:
            ...
            if start.isdigit() and 0 < int(start) <= len(router_matrix):
                dijkstra(start)
    ...
    # Accept the destination router and display the shortest path and cost.
        ...
        if matrix_set == 1:
            ...
            if end.isdigit() and 0 < int(end) <= len(router_matrix):
                if int(start) == 0:
